...
class YoloPredictor(BasePredictor, QObject):
    ...
    def creat_labels(self, detections, img_box, model):
        ...
        # 使用for循环构建labels_draw和labels_write列表
        labels_draw = []
        labels_write = []
        for confidence, class_id, tracker_id, class_name in zip(confidences, class_ids, tracker_ids, class_names):
            labels_draw.append(f"OBJECT-ID: {tracker_id} CLASS: {class_name} CF: {confidence:0.2f}")
            labels_write.append(f"OBJECT-ID: {tracker_id} CLASS: {class_name} CF: {confidence:0.2f}")

        # 如果显示标签 （要有才可以画呀！）---否则就是原图
        if (self.show_labels == True) and (len(tracker_ids) != 0):
